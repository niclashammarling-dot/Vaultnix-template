...
- **Opener cost model (measured 2026-07-25 at 16.7k tokens, 523 articles, composition: 49% concept candidates / 22% MOC Open Territory / 16% ideas / 8% history-layer / 4% lint):** Cost is O(active_candidates × avg_file_size + OT_text_length), NOT O(article_count) or O(session_history). Two interventions ship as defaults in Vaultnix1.0 before the failure mode accumulates: (1) Frontmatter-only candidate reads — Step 1 should read only the first 15 lines of each candidate file (frontmatter + triage state) rather than full file; the agenda displays only slug, triage state, and one-line status; full reads cost ~5,500 chars/file; frontmatter reads cost ~400 chars/file; (2) Open Territory struck-entry archive — struck entries should migrate to a dated archive file at session close rather than accumulating in the live OT text; the working queue should hold only live items. Together these prevent O(candidates + OT) growth from becoming observable to scaffold users at all. `[ready: implement both before first nightly agent deployment]`
