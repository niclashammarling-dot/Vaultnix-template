...
- **Opener cost model (measured 2026-07-25 at 16.7k tokens, 523 articles, composition: 49% concept candidates / 22% MOC Open Territory / 16% ideas / 8% history-layer / 4% lint):** Cost is O(active_candidates × avg_file_size + OT_text_length), NOT O(article_count) or O(session_history). Two interventions ship as defaults in Vaultnix1.0 before the failure mode accumulates: (1) Frontmatter-only candidate reads — Step 1 should read only the first 15 lines of each candidate file (frontmatter + triage state) rather than full file; the agenda displays only slug, triage state, and one-line status; full reads cost ~5,500 chars/file; frontmatter reads cost ~400 chars/file `[ready: implement before first nightly agent deployment]`; (2) ~~Open Territory struck-entry archive~~ Resolved 2026-07-25 in dev vault: struck entries migrate to `wiki/_index/ot-archive.md` at session close via audit-skill OT archive migration step; initial dev vault migration moved 66 entries from 8 MOC files; implement the same audit-skill step and create `wiki/_index/ot-archive.md` before first nightly deployment. `[ready: implement before first nightly agent deployment]`
