...
Read `wiki/_index/SESSION_AGENDA.md`. Check the `Generated:` timestamp:
- If generated by last night's scheduled run or later today: present the file content as-is.
- If stale (older than last night) or file missing: run [[session-agenda-skill]] to regenerate, then present output.
